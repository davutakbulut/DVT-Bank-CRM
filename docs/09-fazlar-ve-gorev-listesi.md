...
- [x] **[G2.01]** Onboarding sihirbazı (4 adım — docs/03 B.0)
- [x] **[G2.02]** Bankalarım CRUD (Livewire + sistem bankası seçici)
- [x] **[G2.03]** Hesaplarım CRUD (KMH alanları dahil)
- [x] **[G2.04]** Kartlarım CRUD (kart mockup görünümü, risk rozeti)
- [x] **[G2.05]** Borçlarım CRUD (sekmeli liste, durum rozetleri)
- [x] **[G2.06]** Ödeme kaydetme akışı (payments_log + remaining düşürme + last_payment_date güncelleme)
- [x] **[G2.07]** Gelir & Gider modülü (kategorili, aylık özet)
- [x] **[G2.08]** DebtCalculator servisi: öncelik skoru, kartopu/çığ karşılaştırması
- [x] **[G2.09]** RiskCounter servisi + scheduled UpdateOverdueCounters job'ı (days_overdue güncelleme)
- [x] **[G2.10]** Dashboard v1: 4 üst kart + yaklaşan ödemeler + borç dağılımı grafiği (AI kartı placeholder)
...
- [x] **[G3.01]** PaymentPlanner servisi: gelir-gider → aylık bütçe → stratejiye göre payment_plan_items üretimi
- [x] **[G3.02]** Plan sihirbazı UI (strateji seçimi + senaryo karşılaştırma + onay)
- [x] **[G3.03]** Plan görünümü: aylık zaman çizelgesi, "ödendi" işaretleme → payments_log entegrasyonu
- [x] **[G3.04]** Takvim ekranı (vadeler + plan ödemeleri + hatırlatıcılar)
- [x] **[G3.05]** Hatırlatıcı motoru: SendReminders job (vade yaklaşanlar: 3 gün & 1 gün önce, email + in-app)
- [x] **[G3.06]** Raporlar: aylık özet, kapanış projeksiyonu grafiği, banka bazlı faiz yükü tablosu
- [x] **[G3.07]** PDF rapor export (DomPDF)
...
- [x] **[G4.01]** AiProviderInterface + Groq provider + Gemini provider + OpenRouter provider + fallback zinciri
- [x] **[G4.02]** UserContextBuilder (anonimleştirilmiş özet JSON — docs/04 şeması)
- [x] **[G4.03]** Kural tabanlı fallback motoru (AI'sız öneri şablonları)
- [x] **[G4.04]** GenerateDailyAdvice job + ai_usage_daily kota kontrolü + plan bazlı frekans (pro günlük / free haftalık)
- [x] **[G4.05]** Dashboard AI kartı (günlük öneri + yenile + kota göstergesi)
- [x] **[G4.06]** AI Koç sayfası: durum analizi + sohbet (son 10 mesaj bağlamı)
- [x] **[G4.07]** Sorumluluk reddi ibaresi tüm AI çıktılarına ekle
- [x] **[G4.08]** Süper admin AI ayar ekranıyla entegrasyon testi (Faz 6'daki ekran gelince tam bağlanır; şimdilik .env)
...
- [x] **[G5.01]** Anasayfa (hero, nasıl çalışır, özellikler, SSS özeti, CTA, footer)
- [x] **[G5.02]** Nasıl Çalışır + Özellikler sayfaları
- [x] **[G5.03]** Fiyatlandırma (plans tablosundan dinamik)
- [x] **[G5.04]** SSS (faqs tablosundan) + İletişim formu (contact_messages)
- [x] **[G5.05]** Yasal sayfalar seed: Gizlilik, KVKK Aydınlatma, Kullanım Şartları, Sorumluluk Reddi + kayıt formuna rıza checkbox'ı
- [x] **[G5.06]** SEO temel: title/meta, og tags, sitemap.xml, robots.txt
...
- [x] **[G6.01]** Filament 3 kurulumu + multi-panel (super + admin) yapılandırması
- [x] **[G6.02]** Admin paneli: Users resource (finansal veri YOK), suspend/aktifleştir
- [x] **[G6.03]** Admin paneli: SupportTickets + TicketMessages (yanıtlama akışı)
- [x] **[G6.04]** Admin paneli: Pages, FAQs, Announcements, ContactMessages resource'ları
- [x] **[G6.05]** Admin dashboard widget'ları: kullanıcı sayıları, kayıt trendi, plan dağılımı (anonim)
- [x] **[G6.06]** Süper admin: yönetici atama, Plans resource
- [x] **[G6.07]** Süper admin: Settings ekranı (gruplu form) + AI ayarları (masked key input, bağlantı testi butonu) + bakım modu
- [x] **[G6.08]** Süper admin: audit log görüntüleyici + failed jobs + import_jobs izleme
- [x] **[G6.09]** Süper admin: manuel yedek tetikleme (mysqldump → storage indirme)
